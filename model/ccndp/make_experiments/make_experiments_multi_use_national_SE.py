...
def make_experiment():
    ...
    emitters = pd.read_csv("/Users/pellemeuzelaar/PycharmProjects/Thesis_V3/model/data/Emitters/SE/AllEmittersBG(t:y).csv")
    emitters = pd.DataFrame(emitters)

    # Facility node data
    conversion = pd.read_csv("/Users/pellemeuzelaar/PycharmProjects/Thesis_V3/model/data/Facilities/SE/SE_ALL_FAC_BG.csv")
    conversion = pd.DataFrame(conversion)

    # Demand node data
    consumption = pd.read_csv("/Users/pellemeuzelaar/PycharmProjects/Thesis_V3/model/data/Demand/SE/Demand_SE_ALL_BG.csv")
    consumption = pd.DataFrame(consumption)

    # Storage node data
    storage = pd.read_csv("/Users/pellemeuzelaar/PycharmProjects/Thesis_V3/model/data/Storage/SE/StorageBG.csv")
    storage = pd.DataFrame(storage)

    # The values inputted for different nodes imported from dataframe
    supply = emitters.iloc[:,4:].values/1000000 # Is in T Convert to Mton
    ...
    for fac in urea_facilities:
        ...
        cost_per_ton_urea = 585730000 #458.47 per ton
        capacity_urea_plants = 1 #Mton
    ...
    for fac in methanol_facilities:
        # Conversion costs in euro per tonne CO2 for each end-use
        cost_per_ton_methanol = 895110000 #700.63 per ton
        capacity_methanol_plants = 1 #Mton
    ...
    for fac in saf_facilities:
        # Conversion costs in euro per tonne CO2 for each end-use
        cost_per_ton_saf = 242740000 #190.00 per ton
        capacity_saf_plants = 1 #Mton
    ...
    for inlayer, outlayer in pairwise([sources, urea_facilities]):
        for frm, to in product(inlayer, outlayer):
            capacity = [0.73] # maximum co2 needed based on the CO2 feedstock required (0.73 Mton for 1 Mton urea)
            distance = geopy.distance.geodesic(frm.loc, to.loc).km
            cost_capital = 24.7*((0.73*31.71)**0.35)*((1000*distance)**1.13)
    ...
    for inlayer, outlayer in pairwise([sources, methanol_facilities]):
        for frm, to in product(inlayer, outlayer):
            capacity = [1.37]
            distance = geopy.distance.geodesic(frm.loc, to.loc).km
            cost_capital = 24.7*((1.37*31.71)**0.35)*((1000*distance)**1.13)
            capex_annual = cost_capital/50
            cost_OM_pipeline = 0.02 * cost_capital # Opex as percentage
            cost = capex_annual + cost_OM_pipeline # + cost_site_capex_opex
            edges.append(Edge(frm, to, cost, capacity, "C"))
    ...
    for inlayer, outlayer in pairwise([sources, saf_facilities]):
        for frm, to in product(inlayer, outlayer):
            capacity = [3.00] # [3.16] Same as supply at source node, not more than the pipeline capacity of 3.5MT
            distance = geopy.distance.geodesic(frm.loc, to.loc).km
            cost_capital = 24.7*((3.00*31.71)**0.35)*((1000*distance)**1.13)
            capex_annual = cost_capital/50
            cost_OM_pipeline = 0.02 * cost_capital # Opex as percentage
            cost = capex_annual + cost_OM_pipeline # + cost_site_capex_opex
            edges.append(Edge(frm, to, cost, capacity, "C"))
    ...
    #
    # cost_site = []
    # # Cost per Mton to store at storage site
    # cost_DOGF_onshore = 1000000  # 1.0 per ton
    # cost_DOGF_offshore = 6400000  # 6.4 euro per ton
    # cost_SA_onshore = 9400000  # 9.4 euro per ton
    # cost_SA_offshore = 30100000  # 30.1 euro per ton
    #
    # for i in range(num_storages):
    #     if storage["Offshore or Onshore"][i] == "Onshore":
    #         if storage["SA or DOGF"][i] == "DOGF":
    #             cost_site.append(cost_DOGF_onshore)
    #     if storage["Offshore or Onshore"][i] == "Offshore":
    #         if storage["SA or DOGF"][i] == "DOGF":
    #             cost_site.append(cost_DOGF_offshore)
    #     if storage["Offshore or Onshore"][i] == "Onshore":
    #         if storage["SA or DOGF"][i] == "SA":
    #             cost_site.append(cost_SA_onshore)
    #     if storage["Offshore or Onshore"][i] == "Offshore":
    #         if storage["SA or DOGF"][i] == "SA":
    #             cost_site.append(cost_SA_offshore)
    # for strg in storages:
    #     cost_site_capex_opex = cost_site[strg.idx - num_sinks - num_facs - num_sources]
    ...
    for inlayer, outlayer in pairwise([sources, storages]):
        for frm, to in product(inlayer, outlayer):
            ...
            distance = geopy.distance.geodesic(frm.loc, to.loc).km
            cost_capital = 24.7*((111.62)**0.35)*((1000*distance)**1.13)
            capex_annual = cost_capital/50
            cost_OM_pipeline = 0.02 * cost_capital # Opex as percentage
            cost = capex_annual + cost_OM_pipeline # + cost_site_capex_opex
            edges.append(Edge(frm, to, cost, capacity, "C"))
...
def main():
    ...
    data.to_file("/Users/pellemeuzelaar/PycharmProjects/Thesis_V3/model/data/Instances/2_MULTI_BG_SA.json")
