...
def main():
    # Opening files
    f = open("/Users/pellemeuzelaar/PycharmProjects/Thesis_V3/model/data/Results/STORAGE_MULTI_NW_saf.json")
    ...
    counter_list = []
    amount_supplied = []
    supplied = emitters["TotalQuantity (kg/year)"]/2
    ...
    for i in range(num_src):
        for j in range(num_sink, num_storage):
            ...
            if data["decisions"][counter] > 0:
                counter_list.append(counter)
                amount_stored.append(emitters["TotalQuantity (kg/year)"][i])
    ...
    total_emissions_not_captured = sum(emitters["TotalQuantity (kg/year)"]) - sum(amount_captured)
    ...
    # Display the values
    print("total emissions by all nodes are: ", sum(emitters["TotalQuantity (kg/year)"]))
    print("Total quantity of CO2 captured with by the emitter building decisions: ", total_emissions_captured)
    print("Total percentage captured: ", total_emissions_captured/sum(emitters["TotalQuantity (kg/year)"]))
    ...
    print("\ncapture costs: ", total_capture_costs/1000000)
    print("transport CO2 costs to facs: ", total_co2_transport_costs/1000000)
    print("conversion costs at facs: ", total_conversion_costs/1000000)
    print("transport costs end-uses (truck): ", total_end_use_transport_costs/1000000)
    print("pipeline costs to storage: ", total_storage_transport_costs/1000000)
    print("\nThe total costs of the supply chain are: ", objective_function/1000000)
